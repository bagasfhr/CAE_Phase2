...
$bod               = $_GET['tgl_lahir']; 
$app_no            = $_GET['app_no'];
$txt_mother            = $_GET['txt_mother'];
$txt_job_modelname     = $_GET['txt_job_modelname'];
$three_or_office       = $_GET['three_or_office'];
$txt_profession_code   = $_GET['txt_profession_code'];
$mode=$_GET['mode'];
$sqlpreapr = ""; 
if ($PreApproval==1) {
  ...
  if ($txt_job_modelname!='') {
    $sqlcustmodel = "SELECT * FROM cc_master_customer_model WHERE cust_model_name LIKE '%".$txt_job_modelname."%' LIMIT 1";
    $rescustmodel = mysqli_query($condb,$sqlcustmodel);
    if($reccustmodel = mysqli_fetch_array($rescustmodel)){
      $txt_job_modelname  = $reccustmodel['cust_model_code'];
    }
  }
    if($txt_profession_code!=''){
        $sqlprof = "SELECT * FROM cc_master_profession WHERE profession_name LIKE '%".$txt_profession_code."%' LIMIT 1";
        $resprof = mysqli_query($condb,$sqlprof);
        if($recprof = mysqli_fetch_array($resprof)){
          $txt_profession_code  = $recprof['profession_code'];
        }
    }
    ...
    $payload = '{
        ...
        "custModel" :"'.$txt_job_modelname.'",
        "motherName" :"'.$txt_mother.'",
        "professionCode" :"'.$txt_profession_code.'",
        "officeCode" :"'.$three_or_office.'",
        "app_no":"'.$app_no.'"
        }';
        if ($customer_id=='') {
          $payload = '{
    ...
    // CURLOPT_URL => 'https://10.0.40.79:8080/cae_score',
    curl_setopt_array($curl, array(
      CURLOPT_URL => 'http://10.0.40.79:8080/cae_score',
...
$sqllog = "INSERT INTO cc_respons_log SET
                ...
                url_api             ='http://10.0.40.79:8080/cae_score', 
                post_api            ='$payload', 
