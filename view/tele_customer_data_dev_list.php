...
}else if (substr($SesExt,0,3) == '631') {
        $SesIP = "https://crmtangcity1.wom.co.id:7675";
}else if (substr($SesExt,0,3) == '632') {
        $SesIP = "https://crmtangcity1.wom.co.id:7675";
}else if (substr($SesExt,0,3) == '731') {
        $SesIP = "https://crmtangcity1.wom.co.id:7675";
}else if (substr($SesExt,0,3) == '732') {
        $SesIP = "https://crmtangcity1.wom.co.id:7675";
}
...
$save_form = "view/telesales/tele_customer_data_dev_new_save.php";
$save_form2= "view/telesales/tele_customer_data_dev_new_save2.php";
$save_form_cust = "view/telesales/tele_customer_data_cust_new_save.php";
...
$sql_whrlist = " AND a.assign_to='$v_agentid' ";
if ($mode=="es") {
	$iddet = "380703"; //"51162"; //diganti ganti aja!
	$wsql  = " AND a.id='$iddet'";
	$sql_whrlist = "";
}
...
if($iddet!=''){
		...
		$txt_spouse_profession		= "";
		$txt_spouse_customer_model	= "";
		$txt_guarantor_name			= "";
		$txt_guarantor_phone		= "";
		$txt_guarantor_prov			= "";
		$txt_guarantor_zip			= "";
		...
		$txt_guarantor_profession	= "";
		$txt_guarantor_customer_model	= "";
		$txt_legal_address			= "";
		$txt_legal_kab				= "";
		$txt_legal_zipcode			= "";
		$txt_legal_rt				= "";
		...
		$txt_asset_type             = "";
		$txt_IS_PRE_APPROVAL        = "";
		$txt_activity_pasangan_ducapil        = "";
		$txt_activity_pasangan_neglist        = "";
		$txt_activity_guarantor_ducapil       = "";
		$txt_activity_guarantor_neglist       = "";
	...
	$sqldet = "SELECT a.id,a.AGRMNT_ID,
					...
					a.guarantor_profession,
					a.guarantor_customer_model,
					a.legal_alamat,
					a.legal_rt,
					a.legal_rw, 
					a.legal_provinsi,
                    ...
                    a.IS_PRE_APPROVAL,
                    a.dukcapil_spouse_stat,
                    a.negative_spouse_cust,
                    a.dukcapil_guarantor_stat,
                    a.negative_guarantor_cust,
                    a.spouse_mother_maiden_name,
                    a.profession_code,
                    a.nama_ibu_kandung,
                    a.notes
				FROM 
	...
	if($recdet = mysqli_fetch_array($resdet)){
		...
		$txt_spouse_mobile_phone		= $recdet['spouse_mobile_phone'];//phoneno_hide($recdet['spouse_mobile_phone']);
		...
		$txt_spouse_profession		= $recdet['spouse_profession'];
		$txt_spouse_customer_model	= $recdet['spouse_customer_model'];
        ...
        $txt_guarantor_profession   = $recdet['guarantor_profession'];
        $txt_guarantor_customer_model = $recdet['guarantor_customer_model'];
		...
		$agrmnt_no3                 = $recdet['agrmnt_no'];
		$marital_status             = $recdet['marital_status'];
		$ltv2                       = $recdet['ltv'];
		$txt_IS_PRE_APPROVAL		= $recdet['IS_PRE_APPROVAL'];
        $txt_activity_pasangan_ducapil      = $recdet['dukcapil_spouse_stat'];
        $txt_activity_pasangan_neglist      = $recdet['negative_spouse_cust'];
        $txt_activity_guarantor_ducapil     = $recdet['dukcapil_guarantor_stat'];
        $txt_activity_guarantor_neglist     = $recdet['negative_guarantor_cust'];
		...
		// NEW CAE 2
        $txt_mother              = $recdet['spouse_mother_maiden_name'];
        $txt_profession_code     = $recdet['profession_code'];
        $txt_mother_cust         = $recdet['nama_ibu_kandung'];
	...
	$sqldet = "SELECT a.id,a.AGRMNT_ID,
                    ...
                    a.negative_guarantor_cust
				FROM 
					cc_ts_penawaran_temp a,
					cc_ts_penawaran_campaign b
				WHERE 
					a.campaign_id=b.id
					AND a.back_flag=1 $whereIs
					AND a.id='$iddet'"; //echo "string2 $sqldet";
					if ($mode=="es") {
						echo "string $sqldet";
					}
					if ($mode=="et") {
	...
	if($recdet = mysqli_fetch_array($resdet)){
		...
		$agrmnt_no2                 = $recdet['agrmnt_no'];
		$marital_status             = $recdet['marital_status'];
		$last_phoneno               = $recdet['last_phoneno'];
		$txt_activity_ducapil       = $recdet['dukcapil_stat'];
		$txt_activity_neglist       = $recdet['negative_cust'];
		...
		$call_status2               = $recdet['call_status'];
		$call_status_sub1           = $recdet['call_status_sub1'];
		$follow_up                  = $recdet['follow_up'];
		$txt_activity_notes         = $recdet['remark_desc'];
		$ltv2                       = $recdet['ltv'];
		$txt_activity_waktuvisit    = $recdet['follow_up'];
		$txt_IS_PRE_APPROVAL		= $recdet['IS_PRE_APPROVAL'];
		...
		$txt_modelname_type = "";
		$sql_cust_type = "SELECT * FROM cc_master_customer_model WHERE cust_model_name LIKE '%".$txt_job_modelname."%' LIMIT 1";
		$res_cust_type = mysqli_query($condb, $sql_cust_type);
		if($row_cust_type = mysqli_fetch_array()){
			$txt_modelname_type = $row_cust_type["cust_type"];
		}

	}
	if ($txt_guarantor_bod!='') {
		$txt_guarantor_bod = date('Y-m-d',strtotime($txt_guarantor_bod));
	}

	// if ($txt_activity_notes=='') {
		$txt_activity_notes = $notes;
	// }
	
		// $txt_IS_PRE_APPROVAL=0;
	...
	$sqlwhr  =" customer_id='$txt_customer_no' OR customer_id_ro='$txt_customer_no' ";
	$sqlwhra =" a.customer_id='$txt_customer_no' OR a.customer_id_ro='$txt_customer_no' ";

	if ($txt_sumber_data=='THIRD PARTY') {
	    $sqlwhr  =" task_id='$txt_task_id' ";
	    $sqlwhra =" a.task_id='$txt_task_id' ";
	}elseif ($txt_customer_no=='') {
	    $sqlwhr  =" task_id='$txt_task_id' ";
	    $sqlwhra =" a.task_id='$txt_task_id' ";
	}
	//a.customer_id='$txt_customer_no' OR a.customer_id_ro='$txt_customer_no'
	$sqldis = "SELECT COUNT(a.id) AS total_main,sum(if(b.call_status>0,1,0)) AS total_temp FROM cc_ts_penawaran a 
			   LEFT JOIN cc_ts_penawaran_temp b
			   ON a.id=b.id
			   WHERE ($sqlwhra) AND a.is_eligible_crm= '1'
			   $sql_whrlist AND a.campaign_id>0";
			   if ($mode=='es') {
			   	echo "</br>--------------</br>string $sqldis";
			   }
			   
	$resdis = mysqli_query($condb,$sqldis);
	if($recdis = mysqli_fetch_array($resdis)){
		$total_main	= $recdis['total_main'];
		$total_temp	= $recdis['total_temp'];
	}

	$paramdate=0;
	$sqljob = "SELECT a.* FROM cc_ts_simulasi a WHERE a.id_cust_detail='$iddet' ORDER BY a.id DESC LIMIT 1"; //echo "sqljob: $sqljob";
	...
	if($recjob = mysqli_fetch_array($resjob)){
		...
		$three_ltv_maks 		= $recjob["three_ltv_maks"]; //echo "three_ltv_maks: ".$three_ltv_maks;
		$three_ltv_yang 		= $recjob["three_ltv_yang"]; //echo "three_ltv_yang: ".$three_ltv_yang;
		if ($ltv2!='') {
			$three_ltv_yang = $ltv2;
		}
		...
		$three_calcu_install	= $recjob["three_calcu_install"];
	}
	...
	$phonecall = "";
	// if($last_phoneno!=''){
	// 	$phonecall  .= "<option value='$last_phoneno' selected>".$last_phoneno."</option>";
	// }
	// if($txt_mobile_no1!=''){
	...
	if($last_phoneno!=''){
		$last_phonenox = phoneno_hide($last_phoneno);
        $phonecall  .= "<option value='$last_phoneno' selected>".$last_phonenox."</option>";
	}
	if($txt_mobile_no1!=''){
		$txt_mobile_no1x = phoneno_hide($txt_mobile_no1);
        $phonecall  .= "<option value='$txt_mobile_no1'>".$txt_mobile_no1x." - Cust Mobile 1</option>";
	}
	if($txt_mobile_phone2!=''){
		$txt_mobile_phone2x = phoneno_hide($txt_mobile_phone2);
        $phonecall  .= "<option value='$txt_mobile_phone2'>".$txt_mobile_phone2x." - Cust Mobile 2</option>";
	}
	if($txt_phone1!=''){
		$txt_phone1x = phoneno_hide($txt_phone1);
        $phonecall  .= "<option value='$txt_phone1'>".$txt_phone1x." - Cust Phone 1</option>";
	}
	if($txt_phone2!=''){
		$txt_phone2x = phoneno_hide($txt_phone2);
        $phonecall  .= "<option value='$txt_phone2'>".$txt_phone2x." - Cust Phone 2</option>";
	}
	if($txt_spouse_phone!=''){
		$txt_spouse_phonex = phoneno_hide($txt_spouse_phone);
        $phonecall  .= "<option value='$txt_spouse_phone'>".$txt_spouse_phonex." - Spouse Phone</option>";
	}
	if($txt_guarantor_phone!=''){
		$txt_guarantor_phonex = phoneno_hide($txt_guarantor_phone);
        $phonecall  .= "<option value='$txt_guarantor_phone'>".$txt_guarantor_phonex." - Guarantor Phone</option>";
	}
	if($txt_guarantor_phone1!=''){
		$txt_guarantor_phone1x = phoneno_hide($txt_guarantor_phone1);
        $phonecall  .= "<option value='$txt_guarantor_phone1'>".$txt_guarantor_phone1x." - Guarantor Phone 1</option>";
	}
	if($txt_guarantor_phone2!=''){
		$txt_guarantor_phone2x = phoneno_hide($txt_guarantor_phone2);
        $phonecall  .= "<option value='$txt_guarantor_phone2'>".$txt_guarantor_phone2x." - Guarantor Phone 2</option>";
	}
	if($txt_job_phone1!=''){
		$txt_job_phone1x = phoneno_hide($txt_job_phone1);
        $phonecall  .= "<option value='$txt_job_phone1'>".$txt_job_phone1x." - Job Phone 1</option>";
	}
	if($txt_job_phone2!=''){
		$txt_job_phone2x = phoneno_hide($txt_job_phone2);
        $phonecall  .= "<option value='$txt_job_phone2'>".$txt_job_phone2x." - Job Phone 2</option>";
	}
	...
	// model customer type
	$sql_cust_model_type = "SELECT DISTINCT(cust_type) FROM cc_master_customer_model WHERE is_active=1";
	$res_cust_model_type = mysqli_query($condb, $sql_cust_model_type);
	...
	// profession
	$sql_profession = "SELECT cmp.id, profession_code, profession_name, cust_model_name, cust_model_code FROM cc_master_profession cmp join cc_master_customer_model cmcm on cmp.cust_model_id=cmcm.id where cmcm.is_active=1";
	$res_profession = mysqli_query($condb, $sql_profession);

	$data_profession_by_custcode=[];
	while($rowProffession = mysqli_fetch_array($res_profession)){
		$data_profession_by_custcode[$rowProffession['cust_model_code']][]=$rowProffession['profession_name'];
	}
...
<!-- NEW -->
<input type="hidden" name="isedd" id="isedd" value="">
						...
						<div class="table-wrapper "><div class="table-responsive">
									<table class="table table-head-bg-primary table-earnings" style="width:100%;height:20px;">
										...
										<tbody>
											...
											$sql_whr2="";
											if ($mode=="es") {
												$sql_whr = " a.is_eligible_crm= '1' AND a.is_process=1 AND (a.customer_id='$txt_customer_no2' OR a.customer_id_ro='$txt_customer_no2') $sql_whr2 ";
											}else{
												if ($txt_sumber_data=='WISE') {
													$sql_whr2=" AND a.assign_to='$v_agentid' ";
												}
												$sql_whr = " a.is_eligible_crm= '1' AND a.is_process=1 AND (a.customer_id='$txt_customer_no2' OR a.customer_id_ro='$txt_customer_no2') $sql_whr2 ";
												if ($txt_customer_no2=="") {
											...
											$arr_cat[] = "";
											$sql_call = "SELECT a.id, a.call_status FROM cc_ts_call_status_cae a";
											$res_call = mysqli_query($condb, $sql_call);
											while($rec_call = mysqli_fetch_array($res_call)) {
											  $arr_cat[$rec_call["id"]] = $rec_call["call_status"];
											}
											mysqli_free_result($res);

											$param_tabel = "cc_ts_penawaran";
											if ($iddet_temp>0) {
												$param_tabel = "cc_ts_penawaran_temp";
											}
											$sqlsa = "SELECT a.*,c.call_status AS call_status2,b.campaign_name,if(a.is_eligible_crm=1,'Eligible','Non Eligible') AS eligible_flag FROM cc_ts_penawaran a
											          LEFT JOIN cc_ts_penawaran_temp c ON a.id=c.id,
													  cc_ts_penawaran_campaign b
													  WHERE $sql_whr
													  AND a.campaign_id=b.id AND a.campaign_id>0";
													  if ($mode=='es') {
													  	echo "</br>list: ".$sqlsa."</br>";
													  }
											 ...
											 while($recsa = mysqli_fetch_array($ressa)){
												...
												$call_status	    = $recsa['call_status2'];
												$completion_data	= $arr_cat[$call_status];
				...
				<div class="card-body">
					<div class="row">
						...
						<div class="col-md-6 col-lg-3">
							...
							<div class="form-group">
								<label for="txtlabel">Spouse Customer Model <span class='required-label neg_label' style="display: none">*</span></label>
								...
								<select class="form-control form-control-sm select2" id="txt_spouse_customer_model" name="txt_spouse_customer_model">
									<option value="" selected>Select Customer Model</option>
									<?php
										while($row = mysqli_fetch_array($res_cust_model)){
											if ($row['cust_model_code'] == $txt_spouse_customer_model) {
												echo "<option value='".$row['cust_model_name']."' data-type='".$row['cust_type']."' data-code='".$row['cust_model_code']."' selected>".$row['cust_model_name']."</option>";
											}else{
												echo "<option value='".$row['cust_model_name']."' data-type='".$row['cust_type']."' data-code='".$row['cust_model_code']."' >".$row['cust_model_name']."</option>";
											}
										}
										mysqli_data_seek($res_cust_model, 0);
									?>
								</select>
							</div>
						</div>
						...
						<div class="col-md-6 col-lg-3">
							...
							<div class="form-group">
								<label for="txtlabel">Spouse Profession</label>
								<select class="form-control form-control-sm select2" id="txt_spouse_profession" name="txt_spouse_profession">
									<option value="" selected>Select Profession</option>
									<?php
										while($row = mysqli_fetch_array($res_profession)){
											if ($row['profession_code'] == $txt_spouse_profession && $txt_spouse_profession != "") {
												echo "<option value='".$row["profession_name"]."' data-code='".$row['cust_model_code']."' selected>".$row["profession_name"]."</option>";
											}elseif($row['cust_model_code']==$txt_spouse_customer_model){
												echo "<option value='".$row["profession_name"]."' data-code='".$row['cust_model_code']."' >".$row["profession_name"]."</option>";
											}
										}
										mysqli_data_seek($res_profession, 0);
									?>
				...
				<div class="card-body">
					<div class="row">
						...
						<div class="col-md-6 col-lg-3">
							...
							<div class="form-group">
								<label for="txtlabel">Guarantor Customer Model <span class='required-label neg_label' style="display: none">*</span></label>
								<select class="form-control form-control-sm select2" id="txt_guarantor_customer_model" name="txt_guarantor_customer_model">
									<option value="" selected>Select Customer Model</option>
									<?php
										while($row = mysqli_fetch_array($res_cust_model)){
											if ($row['cust_model_code'] == $txt_guarantor_customer_model) {
												echo "<option value='".$row['cust_model_name']."' data-type='".$row['cust_type']."' data-code='".$row['cust_model_code']."' selected>".$row['cust_model_name']."</option>";
											}else{
												echo "<option value='".$row['cust_model_name']."' data-type='".$row['cust_type']."' data-code='".$row['cust_model_code']."' >".$row['cust_model_name']."</option>";
											}
							...
							<div class="form-group">
								<label for="txtlabel">Guarantor Profession</label>
								<select class="form-control form-control-sm select2" id="txt_guarantor_profession" name="txt_guarantor_profession">
									<option value="" selected>Select Profession</option>
									<?php
										while($row = mysqli_fetch_array($res_profession)){
											if ($row['profession_code'] == $txt_guarantor_profession && $txt_guarantor_profession != "") {
												echo "<option value='".$row["profession_name"]."' data-code='".$row['cust_model_code']."' selected>".$row["profession_name"]."</option>";
											}elseif($row['cust_model_code']==$txt_guarantor_customer_model){
												echo "<option value='".$row["profession_name"]."' data-code='".$row['cust_model_code']."' >".$row["profession_name"]."</option>";
											}
										}
										mysqli_data_seek($res_profession, 0);
			...
			<div class="card">
				...
				<div class="card-body">
					<div class="row">
						<div class="col-md-6 col-lg-3">
							<div class="form-group">
								<label for="txtlabel">Survey Address  <span class="required-label stat-prospect stat-interest" id="required_survey_addr" style="display:none;">*</span></label>
								<input type="text" class="form-control form-control-sm val-prospect val-interest" id="txt_survey_addr" name="txt_survey_addr" value="<?php echo $txt_survey_addr; ?>">
							</div>
							<div class="form-group">
								<label for="txtlabel">Survey Kabupaten <span class="required-label stat-prospect stat-interest" id="required_survey_addr" style="display:none;">*</span></label>
								<!-- <input type="text" class="form-control form-control-sm" id="txt_survey_kab"  name="txt_survey_kab" value="<?php echo $txt_survey_kab; ?>"> -->
								<select class="form-control form-control-sm select2 val-prospect val-interest" id="txt_survey_kab"  name="txt_survey_kab">
									<option value="" selected>Select Kabupaten</option>
									<?php
							...
							<div class="form-group">
								<label for="txtlabel">Survey Sub Zip Code <span class="required-label stat-prospect stat-interest" id="required_survey_addr" style="display:none;">*</span></label>
								<input type="text" class="form-control form-control-sm val-prospect val-interest" id="txt_survey_subzipcode"  name="txt_survey_subzipcode" value="<?php echo $txt_survey_subzipcode; ?>">
							</div>
						</div>

						<div class="col-md-6 col-lg-3">
							<div class="form-group">
								<label for="txtlabel">Survey RT <span class="required-label stat-prospect stat-interest" id="required_survey_addr" style="display:none;">*</span></label>
								<input type="text" maxlength="3" class="form-control form-control-sm val-prospect val-interest" onkeypress="return isNumberKey(event)" id="txt_survey_rt" name="txt_survey_rt" value="<?php echo $txt_survey_rt; ?>">
							</div>
							<div class="form-group">
								<label for="txtlabel">Survey Kecamatan <span class="required-label stat-prospect stat-interest" id="required_survey_addr" style="display:none;">*</span></label>
								<!-- <input type="text" class="form-control form-control-sm" id="txt_survey_kec" name="txt_survey_kec" value="<?php echo $txt_survey_kec; ?>"> -->
								<select class="form-control form-control-sm select2 val-prospect val-interest" id="txt_survey_kec" name="txt_survey_kec">
									<option value="" selected>Select Kecamatan</option>
									<?php
										if ($txt_survey_kab != "") {
											// kecamatan
							...
							<div class="form-group">
								<label for="txtlabel">House Ownership <span class="required-label stat-prospect stat-interest" id="required_survey_addr" style="display:none;">*</span></label>
								...
								<select class="form-control form-control-sm select2 val-prospect val-interest" id="txt_survey_houseowner" name="txt_survey_houseowner">
						...
						<div class="col-md-6 col-lg-3">
							<div class="form-group">
								<label for="txtlabel">Survey RW <span class="required-label stat-prospect stat-interest" id="required_survey_addr" style="display:none;">*</span></label>
								<input type="text" maxlength="3" class="form-control form-control-sm val-prospect val-interest" onkeypress="return isNumberKey(event)" id="txt_survey_rw" name="txt_survey_rw" value="<?php echo $txt_survey_rw; ?>">
							</div>
							<div class="form-group">
								<label for="txtlabel">Survey Kelurahan <span class="required-label stat-prospect stat-interest" id="required_survey_addr" style="display:none;">*</span></label>
								...
								<select class="form-control form-control-sm select2 val-prospect val-interest" id="txt_survey_kelurahan" name="txt_survey_kelurahan">
									<option value="" selected>Select Kelurahan</option>
									<?php
										if ($txt_survey_kec != "") {
											// kelurahan
							...
							<div class="form-group">
								<label for="txtlabel">Length of Domicile <span class="required-label stat-prospect stat-interest" id="required_survey_addr" style="display:none;">*</span></label>
								<input type="text" class="form-control form-control-sm val-prospect val-interest" id="txt_survey_domicile" name="txt_survey_domicile" value="<?php echo $txt_survey_domicile; ?>">
							</div>
						</div>
						
						<div class="col-md-6 col-lg-3">
							<div class="form-group">
								<label for="txtlabel">Survey Province <span class="required-label stat-prospect stat-interest" id="required_survey_addr" style="display:none;">*</span></label>
								...
								<select class="form-control form-control-sm select2 val-prospect val-interest" id="txt_survey_prov"  name="txt_survey_prov">
									<option value="" selected="">Select Province</option>
									<?php
										while($row = mysqli_fetch_array($res_province)){
							...
							<div class="form-group">
								<label for="txtlabel">Survey Zip Code <span class="required-label stat-prospect stat-interest" id="required_survey_addr" style="display:none;">*</span></label>
								...
								<select class="form-control form-control-sm select2 val-prospect val-interest" id="txt_survey_zipcode" name="txt_survey_zipcode">
									<option value="" selected>Select ZIPCode</option>
									<?php
			...
			<div class="card">
				...
				<div class="card-body">
					<div class="row">
									...
									<?php
										while($row = mysqli_fetch_array($res_cust_model)){
											if ($row['cust_model_name'] == $txt_job_modelname) {
												echo "<option value='".$row['cust_model_name']."' data-code='".$row['cust_model_code']."' data-type='".$row['cust_type']."' selected>".$row['cust_model_name']."</option>";
											}else{
												echo "<option value='".$row['cust_model_name']."' data-code='".$row['cust_model_code']."' data-type='".$row['cust_type']."' >".$row['cust_model_name']."</option>";
											}
										}
										mysqli_data_seek($res_cust_model, 0);
									?>
						...
						<div class="col-md-6 col-lg-3">
							<div class="form-group">
								<label for="txtlabel">Customer Model Type</label>
								<!-- <input type="text" class="form-control form-control-sm" id="txt_job_modelname" name="txt_job_modelname" value="<?php echo $txt_job_modelname; ?>"> -->
								<select class="form-control form-control-sm select2" id="txt_modelname_type" name="txt_modelname_type">
									<option value="" selected>Select Customer Type</option>
									<?php
										while($row = mysqli_fetch_array($res_cust_model_type)){
											$label = "Personal";
											if($row['cust_type'] == "C"){
												$label = "Company";
											}
											if ($row['cust_type'] == $txt_modelname_type) {
												echo "<option value='".$row['cust_type']."' selected>".$label."</option>";
											}else{
												echo "<option value='".$row['cust_type']."' >".$label."</option>";
											}
										}
										mysqli_data_seek($res_cust_model_type, 0);
									?>
								</select>
							</div>
							<div class="form-group">
									...
									<?php
										while($row = mysqli_fetch_array($res_profession)){
											if ($row['profession_name'] == $txt_job_profname && $txt_job_profname != "") {
												echo "<option value='".$row["profession_name"]."' data-code='".$row['cust_model_code']."' selected>".$row["profession_name"]."</option>";
											}elseif($row['cust_model_name']==$txt_job_modelname){
												echo "<option value='".$row["profession_name"]."' data-code='".$row['cust_model_code']."' >".$row["profession_name"]."</option>";
											}
										}
										mysqli_data_seek($res_profession, 0);
							...
							<div class="form-group">
								<?php
					        		...
					        		function get_select_master_lob($conDB, $id, $name, $required, $mlob,$asset_type,$param_lob) {
										...
										$sql_str1 = " SELECT a.lob_code, a.lob_name FROM cc_master_pot a $sql_whr GROUP BY a.lob_code ";
										$sql_res1  = execSQL($conDB, $sql_str1);
										while ($sql_rec1 = mysqli_fetch_array($sql_res1)) {
		        					...
		        					function get_select_master_product_offer($conDB, $id, $name, $required, $product_id, $mlob) {
											$sel = "<SELECT id=\"$id\" name=\"$name\" class=\"form-control form-control-sm select2 val-prospect val-interest\" style=\"width:100%;\" \"$required\" >";//disabled
											...
											$sql_str1 = " SELECT a.id, a.prod_offering_code, a.prod_offering_name, b.lob_name FROM cc_prod_offering a
														 LEFT JOIN cc_master_pot b ON a.prod_offering_code=b.prod_offering_code
		                    		...
		                    		$txtlabel[$x]      = "Product Offering <span class='required-label stat-prospect stat-interest' style='display:none;'>*</span>";
							...
							<div class="form-group">
								<?php
		                    		...
		                    		$ltvnya = "
										<div class=\"input-group mb-3\">
											".
											input_text_temp("three_ltv_maks","three_ltv_maks",$three_ltv_maks,"","","form-control form-control-sm val-prospect","")."
											<div class=\"input-group-append\">
												<span class=\"input-group-text\"  >%</span>
											</div>
										</div>";

		                    		$txtlabel[$x]      = "LTV  Maksimal <span class='required-label stat-prospect' style='display:none;'>*</span>";
						...
						<div class="col-md-6 col-lg-3">
							<div class="form-group">
								<?php
									...
									$seltenor = "<select name='three_tenor' id='three_tenor' class='form-control form-control-sm select2 val-prospect'>";
									if($three_tenor > 0) {
										for ($i=1; $i <= $three_tenor; $i++) { 
											if($three_tenor == $i) {
												$seltenor .= "<option value='".$i."' selected>".$i."</option>";
											} else {
												$seltenor .= "<option value='".$i."'>".$i."</option>";
											}
										}
									} else {
										$seltenor .= "<option value='0'>0</option>";
									}
									
									$seltenor .= "</select>";
		        					
		                    		$txtlabel[$x]      = "Tenor <span class='required-label stat-prospect' style='display:none;'>*</span>";
		                    		$bodycontent[$x]   = $seltenor;
									//." </br>".input_text_temp_keypress("three_tenor","three_tenor",$three_tenor,"","","form-control form-control-sm ","return validate_number(event)");
		                    		$x++;
	                    ...
	                    <div class="row">
	                        <div class="col-md-6 col-lg-6">
	                            <div class="form-group">
	                                ...
	                                <select name='txt_activity_result' id='txt_activity_result' class='form-control form-control-sm'>
	                                    ...
	                                    <?php 
	                                        $sqlcs = "SELECT id, call_status FROM cc_ts_call_status_cae ORDER BY call_status ASC ";//WHERE id > 2 
	                                    ...
	                                    ?>
	                                        		<option value="<?php echo $id;?>" selected><?php echo $cstatus; ?></option>;
	                                    <?php 
	                                        	}else{
	                                    ?>
	                                ...
	                                <select name='txt_activity_substatuscall' id='txt_activity_substatuscall' class='form-control form-control-sm'>
	                                    ...
	                                    <?php 
	                                        $sqlcs = "SELECT id, call_status_sub1 
	                                                          FROM cc_ts_call_status_cae_sub1
	                        ...
	                        <div class="col-md-6 col-lg-6">
	                            <div class="form-group">
	                                ...
	                                <input type="text" maxlength="200" class="form-control form-control-sm" id="txt_activity_notes" name="txt_activity_notes" value="<?php echo $txt_activity_notes; ?>" maxlength="100">
	                            </div>
	                            <div class="form-group">
	                            ...
	                            <?php 
	                            	// if ($txt_IS_PRE_APPROVAL=='1') {
	                            ?>
	                            <div class="form-group" id="div_opsi" name="div_opsi" style="display: <?php echo $dis_opsi; ?>">
								...
								<div class="form-group" id="div_followup" style="display: none;">
									<label for="txtlabel">Follow Up <span class="required-label" id="required_followup" style="display:none;">*</span></label>
										<input type="text" class="form-control" id="txtfollowup" name="txtfollowup"  value="<?php echo $txtfollowup;?>">
								</div>
	                            <div class="form-group">
	                                <label for="txtlabel">&nbsp;</label>
	                                <?php 
	            $btnduckcapil = "<input type='button' name='btn_duckcapil' id='btn_duckcapil' class='btn btn-secondary btn-sm' value='Validation Check'>";//Check Dukcapil
	            echo $btnduckcapil;
	                    ?>
	                            </div>
	                        </div>
	                        ...
	                        <div class="card-action">
	                            <button class="btn btn-success" id="btnSaveForm" disabled>Submit</button>
...
<!--   Core JS Files   -->
	...
	$(document).ready(function(){
		...
		/* 
		START HIDE PROFESSION BASED ON CUST MODEL
		*/
			const professionByCustCode  = <?php echo json_encode($data_profession_by_custcode) ?>;

			/**
			 * Update Select2 options based on code
			 * @param {string|null} code
			 */
			function updateProfessionSelect(custCode,select2id) {
			    const $select = $(select2id);

			    // clear previous options
			    $select.empty().trigger('change');

			    // if no data
			    if (!custCode || !professionByCustCode[custCode]) {
			    ...
			    const defaultOption = new Option(
			     ...
			     $select.append(defaultOption);
			    // add options
			    professionByCustCode[custCode].forEach(name => {
			...
			//if spouse cust model change 
			$('#txt_spouse_customer_model').on('change', function (){
					const custCode = $(this).find(':selected').data('code');
					updateProfessionSelect(custCode,'#txt_spouse_profession');
			});

			//if Guarantor cust model change 
			$('#txt_guarantor_customer_model').on('change', function (){
					const custCode = $(this).find(':selected').data('code');
					updateProfessionSelect(custCode,'#txt_guarantor_profession');
			});

			//if job info cust mode change
			$('#txt_job_modelname').on('change', function (){
					const custCode = $(this).find(':selected').data('code');
					updateProfessionSelect(custCode,'#txt_job_profname');
			});
		/* 
		END HIDE PROFESSION BASED ON CUST MODEL
		*/
		var pro_offering    = $("#three_pro_offering").val();
		var or_office = "";
		try {
			var data = $('#three_or_office').find(':selected').text();
			if(data) {
				var str_office = data.split(" / ");
			  	// console.log(str_office[0]);
			  	or_office = str_office[0];
			}
		} catch(err) { }

		var dataString = 'pro_offering='+pro_offering+'&or_office='+or_office+'&v=pro_offering2_api';
		// $.ajax({ 
		//     type: 'POST', 
		...
		// 		document.getElementById("param_pro_offering").value = data.param_pro_offering;
		// 		$('#three_tenor').html(data.three_tenor_api);
		//     }
			      
		// });

		//OTR
		var source_gdata    = $("#txt_sumber_data").val();
		if(source_gdata == "WISE") {
			var otr_mfr_year = $("#three_mfr_year").val();
			var otr_office = "";
			try {
				var data = $('#three_or_office').find(':selected').text();
				if(data) {
					var str_office = data.split(" / ");
				  	// console.log(str_office[0]);
				  	otr_office = str_office[0];
				}
			} catch(err) { }

			var otr_asset= "";
			try {
				var data = $('#three_asset_name').find(':selected').text();
				if(data) {
					var str_asset = data.split(" / ");
				  	// console.log(str_asset[0]);
				  	otr_asset = str_asset[0];
				}
			} catch(err) { }

			// console.log(otr_mfr_year+" && "+otr_office+" && "+otr_asset);

			// var dataString = 'otr_office='+otr_office+'&otr_asset='+otr_asset+'&otr_year='+otr_mfr_year+'&v=otr_api';
			// $.ajax({ 
			//     type: 'POST', 
			//     url: 'view/telesales/get_data_value.php', 
			//     data: dataString, 
			//     dataType:'json',
			//     success: function (data) {
			// 		document.getElementById("three_otr").value = data.data_otr;
			//     }
		...
		if(call_status==1) {
			var stat = document.getElementsByClassName('stat-interest');
			$('.val-interest').attr('required', false)[0]; 
			...
			var stat = document.getElementsByClassName('stat-interest');
			var sval = document.getElementsByClassName('val-interest');
			$('.val-interest').attr('required', true)[0]; 
			for (let index = 0; index < stat.length; index++) {
				stat[index].style.display = "contents";
			}
	...
	$("#three_or_office").change(function() {
		var source_gdata    = $("#txt_sumber_data").val();
		if(source_gdata == "WISE") {
			var otr_mfr_year = $("#three_mfr_year").val();
			var otr_office = "";
			...
			var dataString = 'pro_offering='+pro_offering+'&or_office='+otr_office+'&v=pro_offering2_api';
			$.ajax({ 
			    type: 'POST', 
			    url: 'view/telesales/get_data_value.php', 
			    data: dataString, 
	...
	$("#three_asset_name").change(function() {
		var source_gdata    = $("#txt_sumber_data").val();
		if(source_gdata == "WISE") {
			var otr_mfr_year = $("#three_mfr_year").val();
			var otr_office = "";
	...
	$('#txt_customer_name').on('input', function() {
...
if(txtphonenoadd1!='' )
	...
	$("#btncallcust").click(function(){
		...
		$.ajax({
			...
			data:'ref=' + ref + '&iddet=' + iddet + '&direction=' + direction + '&agentid='+v_agentid+'&form_id='+form_id,
    ...
    $("#btnSaveForm").click(function(){ 
		...
		var param_professionCode     = document.getElementById("txt_job_profname").value;
	 	...
	 	if(param_neglist == "Match" || param_neglist=="MATCH"){
			$("#txt_activity_ducapil").prop("required", false);
		}
	 	// if (param_dukcapil2=='NULL'||param_dukcapil2=='') {
	 	// 	fvalid=false;
	 	// 	param_warm = "Please Check Dukcapil";
	 	// }
    	 ...
    	 // var url = "https://devcrm.wom.co.id/wom/index.php?v=dGVsZXNhbGVzfHRlbGVfY3VzdG9tZXJfZGF0YV9kZXZfbGlzdHxDdXN0b21lciBEYXRhIERldnwzMDZ8";
    	 // var url = "https://devcrm.wom.co.id/wom/index.php?v=dGVsZXNhbGVzfHRlbGVfY3VzdG9tZXJfZGF0YV9kZXZfbGlzdHxDdXN0b21lciBEYXRhIERldnwzMDZ8";
    	 var url = "https://devcrm.wom.co.id/wom/index.php?v=dGVsZXNhbGVzfHRlbGVfY3VzdG9tZXJfZGF0YV9kZXZfbGlzdHxDdXN0b21lciBEYXRhIERldnwzMDZ8";  
    	 ...
    	 var txt_spouse_name= document.getElementById("txt_spouse_name").value;
    	 if (txt_spouse_name !="") {
    	 	var param_spousename= document.getElementById("txt_spouse_customer_model").value;
    	 	if (param_spousename =="") {
    	 		fvalid=false;
	    	 	param_warm = "Please choice Spouse Customer Model";
				$("#txt_spouse_customer_model").attr("required", "required");
    	 	}
    	 	var param_spouseprof= document.getElementById("txt_spouse_profession").value;
    	 	if (param_spouseprof =="") {
    	 		fvalid=false;
	    	 	param_warm = "Please choice Spouse Profession";
				$("#txt_spouse_profession").attr("required", "required");
    	 	}
    	 }

    	 var txt_guarantor_name= document.getElementById("txt_guarantor_name").value;
    	 if (txt_guarantor_name !="") {
    	 	var param_guarantorname= document.getElementById("txt_guarantor_customer_model").value;
    	 	if (param_guarantorname =="") {
    	 		fvalid=false;
	    	 	param_warm = "Please choice Guarantor Customer Model";
				$("#txt_guarantor_customer_model").attr("required", "required");
    	 	}
    	 	var param_guarantorprof= document.getElementById("txt_guarantor_profession").value;
    	 	if (param_guarantorprof =="") {
    	 		fvalid=false;
	    	 	param_warm = "Please choice Guarantor Profession";
				$("#txt_guarantor_profession").attr("required", "required");
    	 	}
    	 }

    	 //check customer mandatory
    	 var txt_customer_name= document.getElementById("txt_customer_name").value;
    	 if (txt_customer_name !="") {
    	 	var param_customername= document.getElementById("txt_job_modelname").value;
    	 	if (param_customername =="") {
    	 		fvalid=false;
	    	 	param_warm = "Please choice Customer Model";
				$("#txt_job_modelname").attr("required", "required");
    	 	}
    	 	var param_customerprof= document.getElementById("txt_job_profname").value;
    	 	if (param_customerprof =="") {
    	 		fvalid=false;
	    	 	param_warm = "Please choice Customer Profession";
				$("#txt_job_profname").attr("required", "required");
    	 	}
    	 }
	    	 ...
	    	 $.ajax({
				...
				success: function(data){ 
					...
					if (datares3=='NOT ELIGIBLE WSC') {
						fvalid=false;
	    	 			param_warm = "Warning Notification: NOT ELIGIBLE WSC";
						            	...
						            	if(warn=="Success!") {
											...
											// 				else{
	    	 													...
	    	 													if (taskid != "") {
																	...
																	$.ajax({
																			...
																			success: function(data){ 
																				...
																				}else if(datares=="STATE_NOT_TASK_SLA" || datares=="Error Occured"){
																					// alert('state');
													    ...
													    //         	var urlinsertpolo  = "service/wom/api_fin_insert_data_new.php?agrmntid="+agrmntid;
																	...
																	// 			var datares3 = data.data;
																	// 			if (datares=='SUCCESS') {
																	//             	var urlupdatepolo  = "service/wom/api_fin_update_cust_data_new.php?taskId="+datares3;
																	// 				$.ajax({
																	// 						url: urlupdatepolo,
																	// 						type: "POST",
																	// 						dataType: 'json',
	...
	$("#txt_activity_result").change(function() {
			  ...
			  var dataString = 'call_statusid='+call_status+'&v=call_status';
			  	...
			  	if(call_status==1) {
			  		// reset interest
			  		var stat = document.getElementsByClassName('stat-interest');
					$('.val-interest').prop('required', false); 
					for (let index = 0; index < stat.length; index++) {
						stat[index].style.display = "none";
					}
					...
					var stat = document.getElementsByClassName('stat-interest');
					var sval = document.getElementsByClassName('val-interest');
					$('.val-interest').prop('required', true); 
					for (let index = 0; index < stat.length; index++) {
						stat[index].style.display = "contents";
					}
				} else{
			  ...
			  var param_disable = 0;
			  var tot_main = <?php echo $total_main; ?>;
			  var tot_temp = <?php echo $total_temp; ?>;
			  var cust_temp = "<?php echo $txt_customer_no2; ?>";
			  var tot_tot = tot_main-tot_temp;
			  
			  if (tot_main-tot_temp==1 || tot_main==1) {
			  	param_disable=1;
			  }else{
			  	if (call_status!=5||call_status!=6) {
			  		param_disable=0;
			  	}
			  }
			  ...
			  if (call_status==4) {
			  	$("#div_followup").show();
			  	$("#txtfollowup").prop("required", true);
			  	$("#required_followup").show();
			  }else{
			  	$("#txtfollowup").prop("required", false);
			  	$("#required_followup").hide();
			  	$("#div_followup").hide();
			  }
				$.ajax({ 
			      type: 'POST', 
	...
	$("#three_pro_offering").change(function() {
		...
		$("#btn_duckcapil").click(function() {
			...
			// NEW CAE 2
			var txt_mother              = document.getElementById("txt_mother").value;
			var txt_mother_cust         = document.getElementById("txt_mother_cust").value;
			...
			if (param_dukcapil==0) {
				...
				var urlcheckcust	   = "service/wom/check_negative.php?nik="+nik+"&nama_lengkap="+nama_lengkap+"&tempat_lahir="+tempat_lahir+"&tgl_lahir="+tgl_lahir+"&customer_id="+cust_no+"&source="+source+"&PreApproval="+IS_PRE_APPROVAL+"&app_no="+app_no+"&txt_mother="+txt_mother_cust+"&txt_job_modelname="+txt_job_modelname+"&three_or_office="+three_or_office+"&txt_profession_code="+txt_profession_code;
					$.ajax({
							...
							success: function(data2){ 
								...
								document.getElementById("isedd").value = datarnegres3;
							    ...
							    var param_marital= document.getElementById("marital_status").value;
							    	// param_marital 		= "MENIKAH"; // testing
							    	datarnegres3 		= "NO"; // testing
								    // if (param_marital=='MENIKAH' && datarnegres3=='YES') {
								    // 	// jadi mandatory txt_customer_model_spouse dan txt_customer_model_gurantor
								    // 	$(".neg_label").css("display", "contents");
								    // 	$("#txt_spouse_customer_model").attr("required", "required");
								    // 	$("#txt_guarantor_customer_model").attr("required", "required");
								    // }else{
								    // 	// tidak mandatory customer_model_spouse dan customer_model_gurantor
								    // 	$(".neg_label").css("display", "none");
								    // 	$("#txt_spouse_customer_model").attr("required", false);
								    // 	$("#txt_guarantor_customer_model").attr("required", false);
								    // }
				...
				if (spouse_nik!='') {
					var txt_profession_code     = document.getElementById("txt_spouse_profession").value;
					var txt_job_modelname       = document.getElementById("txt_spouse_customer_model").value;
					var urlcheckcust2	   = "service/wom/check_negative.php?nik="+spouse_nik+"&nama_lengkap="+spouse_name+"&tempat_lahir="+spouse_bod_place+"&tgl_lahir="+spouse_bod+"&source="+source+"&PreApproval="+IS_PRE_APPROVAL+"&app_no="+app_no+"&txt_mother="+txt_mother+"&txt_job_modelname="+txt_job_modelname+"&three_or_office="+three_or_office+"&txt_profession_code="+txt_profession_code;					setTimeout(function() { 
					$.ajax({
							url: urlcheckcust2,
							type: "POST",
							dataType: 'json',
							success: function(data2){ 
								...
								datarnegres      = datarnegres.replace("CUstomer", "Customer");
								// alert(datarnegres);
								if (datarnegres == 'MATCH - FATAL') {//Match
				...
				if (guarantor_nik!='') {
					var txt_profession_code     = document.getElementById("txt_guarantor_profession").value;
					var txt_job_modelname       = document.getElementById("txt_guarantor_customer_model").value;
					var urlcheckcust3	   = "service/wom/check_negative.php?nik="+guarantor_nik+"&nama_lengkap="+guarantor_name+"&tempat_lahir="+guarantor_bod_place+"&tgl_lahir="+guarantor_bod+"&source="+source+"&PreApproval="+IS_PRE_APPROVAL+"&app_no="+app_no+"&txt_job_modelname="+txt_job_modelname+"&three_or_office="+three_or_office+"&txt_profession_code="+txt_profession_code;
					setTimeout(function() {
					$.ajax({
							url: urlcheckcust3,
							type: "POST",
		...
		$("#txt_modelname_type").change(function(){
			$("#txt_job_modelname span option").unwrap()
			type = $(this).val();
			if (type == "") {
				$("#txt_job_modelname span option").unwrap();
			}else{
				modelname 		= "<?php echo $txt_job_modelname ?>";
				modelname_temp 	= $("#txt_job_modelname").val();
				if (modelname != modelname_temp) {
					$("#txt_job_modelname").val("").trigger("change");
				}

				$("#txt_job_modelname option").wrap( '<span class="toggleOption" style="display: none;" />' );
				$("#txt_job_modelname option[data-type="+type+"]").unwrap();
				$("#txt_job_modelname option[value='']").unwrap();
			}
		});
	...
	<script>
		$( document ).ready(function() {
			...
			var urlduplicate	   = "service/wom/check_duplicate.php?nik="+nik+"&nama_lengkap="+nama_lengkap+"&tempat_lahir="+tempat_lahir+"&tgl_lahir="+tgl_lahir; 
			if (nik!='') {
				console.log("check duplicate: 5041")
				document.getElementById('tempatLoading').style.display = "";
		    	 $.ajax({
					url: urlduplicate,
					type: "POST",
					...
					success: function(data){ 
						...
						if (datares3 !=null) {
							...
							var cabang = "";
							if (datares_cabang !='') {
								cabang = datares_cabang.replaceAll('</br>', '\n');
							}
							...
							if (datares6=='MATCH') {
								...
								.then((willDelete) => {
								  if (willDelete) {
									...
									// alert('xx2');agrmnt_no
									var param_agrmnt_no= "<?php echo $agrmnt_no3; ?>";//document.getElementById("agrmnt_no").value;
									var urlcustnik	   = "service/wom/check_custnik.php?nik="+nik;
									$.ajax({
										url: urlcustnik,
										type: "POST",
										dataType: 'text',
										success: function(data2){ 
											// var datares  = data.duplicateStatus;
											//alert(data2);
											const arr_data = data2.split(",");
											for (let i = 1; i < arr_data.length; i++) {
											    // console.log(arr_data[i]);
											    // alert(arr_data[i]);
											    const arr_data2 = arr_data[i].split("|");
											    // alert(arr_data2[0]+"||"+param_agrmnt_no);
											    if (arr_data2[0]==param_agrmnt_no) {
											    	if (arr_data2[1]==1) {
											    		document.getElementById('div_opsi').style.display = "block";
														document.getElementById('txt_IS_PRE_APPROVAL').value='1';
											    	}else{
											    		document.getElementById('div_opsi').style.display = "none";
														document.getElementById('txt_IS_PRE_APPROVAL').value='0';
											    	}
											    }
											}
		...
		function formatRupiah(angka, prefix)
    {
		...
		var xvalid = false; var xbefore = 0;
		$('input[name=check_assign]').mouseup(function(){
			xbefore = $('input[name=check_assign]:checked').val();
			xvalid = form.valid();
		}).change(function(){
			if(xvalid == false) {
				$("input[name=check_assign][value=" + xbefore + "]").prop('checked', true);
			}
		});
     ...
     //    	}
			    ...
			    success: function(d) {
		        	...
		        	for (var i = 0; i < row_bulk.length; i++) {
		            	if (row_bulk[i].checked == true) {
		            		...
		            		var url = "https://devcrm.wom.co.id/wom/index.php?v=dGVsZXNhbGVzfHRlbGVfY3VzdG9tZXJfZGF0YV9kZXZfbGlzdHxDdXN0b21lciBEYXRhIERldnwzMDZ8";   
		            		var url = "https://devcrm.wom.co.id/wom/index.php?v=dGVsZXNhbGVzfHRlbGVfY3VzdG9tZXJfZGF0YV9kZXZfbGlzdHxDdXN0b21lciBEYXRhIERldnwzMDZ8";
		            		// var url = "https://devcrm.wom.co.id/wom/index.php?v=dGVsZXNhbGVzfHRlbGVfY3VzdG9tZXJfZGF0YTJfZGV2X2xpc3R8Q3VzdG9tZXIgRGF0YSBEZXYgU3lzfDMwOXw%3D"; 
							url += '&param='+row_bulk[i].value;
							window.location.href = url; 
		             	}
		             	else{
		...
		$("#marital_status").change(function() {
			...
			// No | Kondisi Pemohon        | Cust Model & Profesi Pemohon | Cust Model & Profesi Pasangan | Cust Model & Profesi Penjamin
			// ---|------------------------|------------------------------|-------------------------------|------------------------------
			// 1  | Single / Widow         | YES                          | NO                            | NO
			// 2  | Married                | YES                          | YES                           | NO
			// 3  | Single Need Guarantor  | YES                          | NO                            | YES
			// 4  | Married Need Guarantor | YES                          | YES                           | YES

		    var param_marital2 = param_marital.toLowerCase();
			if (param_marital2=='single' && param_marital2=='widow') {
				// jadi mandatory txt_customer_model_spouse dan txt_customer_model_gurantor
				$(".neg_label").css("display", "none");
				$("#txt_spouse_customer_model").attr("required", false);
				$("#txt_guarantor_customer_model").attr("required", false);
			} else if (param_marital2=='menikah' && param_marital2=='merried') {
				$(".neg_label").css("display", "contents");
				$("#txt_spouse_customer_model").attr("required", "required");
				$("#txt_guarantor_customer_model").attr("required", false);
			}  else if (param_marital2=='single need guarantor') {
				$(".neg_label").css("display", "contents");
				$("#txt_spouse_customer_model").attr("required", false);
				$("#txt_guarantor_customer_model").attr("required", "required");
			} else{
	...
	$("#three_ltv_maks").keypress(function(evt){
	...
	$("#three_ltv_yang").keypress(function(evt){
	  var charCode = (evt.which) ? evt.which : evt.keyCode;
          if (charCode != 46 && charCode > 31 
            && (charCode < 48 || charCode > 57))
             return false;
